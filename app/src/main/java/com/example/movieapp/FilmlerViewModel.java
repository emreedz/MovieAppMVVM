...
public class FilmlerViewModel extends ViewModel {
    ...
    public LiveData<List<FilmlerResponse>> getFilmlerListeObserver() {
        return filmlerListe;
    }

    public void ApiCall() {
        Call<List<FilmlerResponse>> callFilmler = RetrofitClient.getFilmler().filmlerCall(page);
        callFilmler.enqueue(new Callback<List<FilmlerResponse>>() {
            @Override
